    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm '>
      ...
      <div className=''>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700 '>
          ...
          <p className=' text-green-500 tracking-wide'>{userData.email}</p>
          <p className='font-medium'>Phone :</p>
          {
              isEdit 
              ? <input className='bg-gray-100 max-w-52' type="text" value={userData.phone} onChange={(e) => setUserData((prev) => ({...prev , phone : e.target.value}))} />
              : <p className=' text-green-500 tracking-wide'>{userData.phone}</p>
           }
        ...
        <div className=''>
           ...
           <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700 '>
            ...
            {
              isEdit 
              ? <select className='max-w-20 bg-gray-50'  onChange={(e)=> setUserData(prev => ({...prev  , gender : e.target.value}))} value={userData.value} >
      ...
      <div className='mt-10'>
        {
          isEdit 
          ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all duration-500' onClick={(e)=>{updateUserProfileData()}}>Save information</button>
          : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all duration-500' onClick={(e)=>{setIsEdit(true)}}>Edit</button>
        }
      </div>
